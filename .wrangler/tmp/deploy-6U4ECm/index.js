...
var RoomDurableObject = class {
  ...
  constructor(state, env) {
    ...
    this.messages = [];
    this.callSignals = [];
    // Store pending call signals for polling
    ...
    this.state.blockConcurrencyWhile(async () => {
      ...
      const storedSignals = await this.storage.get("callSignals");
  ...
  async fetch(request) {
    ...
    if (path.startsWith("/api/poll")) {
      ...
      if (method === "get_messages") {
        ...
        const relevantSignals = this.callSignals.filter(
          (sig) => sig.data.callee?.id === userId || sig.data.caller?.id === userId
        );
      ...
      const now = Date.now();
      this.callSignals = this.callSignals.filter((sig) => now - (sig.timestamp || 0) < 30 * 60 * 1e3);
  ...
  async handleCallSignal(userId, data) {
